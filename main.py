...
def wait_for_vns_to_sync():
    print("Waiting for VNs to sync to",
          base_node.grpc_base_node.get_tip(), end=" ")
    # We have to check if VNs are already running their jrpc server
    while True:
        try:
            all(
                vn.jrpc_client.get_epoch_manager_stats(
                )["current_block_height"] != base_node.grpc_base_node.get_tip() - 3
        ...
        for file in os.listdir(os.getcwd()):
            ...
            if os.path.isdir(full_path):
                if DELETE_EVERYTHING_BEFORE:
                    if re.match(
                        r"(config|data|base_node|log|peer_db|indexer|miner|vn\d+|wallet|dan_wallet_daemon|dan_wallet_daemon\d+|templates|stdout)", file
                    ):
        ...
        while True:
            ...
            try:
                if command == "help":
                    ...
                    print(
                        "jrpc <vn <id>|dan <id>|indexer> - to get jrpc port of vn with id <id>, dan wallet with id <id> or indexer")
                    print(
                        "http <vn <id>|dan <id>|indexer> - to get http address of vn with id <id>, dan with id <id> or indexer")
                    print(
                    ...
                    print(
                        "live - list of things that are still running from this python (base node, wallet, ...)")
                    print("---")
                    print("The VNs are zero based index")
                elif command.startswith("burn"):
                    public_key = command.split()[1]
                    public_key = bytes(
                        int(public_key[i: i + 2], 16) for i in range(0, len(public_key), 2))
                    print(f"### BURNING {BURN_AMOUNT} ###")
                    burn = wallet.grpc_client.burn(BURN_AMOUNT, public_key)
                    os.mkdir("output")
                    outfile = "burn.json"
                    ...
                    if r := re.match("jrpc vn (\d+)", command):
                        ...
                        if vn_id in VNs:
                            ...
                            print(
                                f"VN id ({vn_id}) is invalid, either it never existed or you already killed it")
                    ...
                    if command.startswith("http vn"):
                        ...
                        if r := re.match("http dan (\d+)", command):
                            ...
                            if dan_id in DanWallets:
                                print(
                                    f"http://localhost:{DanWallets[dan_id].http_client.http_port}")
                            else:
                                print(
                                    f"DAN id ({vn_id}) is invalid, either it never existed or you already killed it")
                    ...
                    match what:
                        case "node":
                            print(
                                f'To run base node : {base_node.exec.replace("-n ", "")}')
                            del base_node
                        case "wallet":
                            print(
                                f'To run the wallet : {wallet.exec.replace("-n ", "")}')
                            del wallet
                        case "indexer":
                            print(
                                f'To run the indexer : {indexer.exec.replace("-n ", "")}')
                            del indexer
                        case _:
                            ...
                            if r := re.match("vn (\d+)", what):
                                ...
                                if vn_id in VNs:
                                    ...
                                    print(
                                        f"Command that was used to register the VN : {VNs[vn_id].exec_cli}")
                                    del VNs[vn_id]
                                else:
                                    print(
                                        f"VN id ({vn_id}) is invalid, either it never existed or you already killed it")
                                ...
                                if dan_id in DanWallets:
                                    print(
                                        f"To run the vn : {DanWallets[dan_id].exec}")
                                    del DanWallets[dan_id]
                                else:
                                    print(
                                        f"DanWallet id ({dan_id}) is invalid, either it never existed or you already killed it")
                            else:
                    ...
                    template.call_function(DEFAULT_TEMPLATE_FUNCTION, next(
                        iter(VNs.values())).json_rpc_port)
